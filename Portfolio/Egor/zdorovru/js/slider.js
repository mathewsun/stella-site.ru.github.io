...
function slider(target, showfirst) {
    ...
    for (i = 0; i < divslength; i++) {
        ...
        if (divid.indexOf("header") != -1) {
            //div.getElementsById('goodlink')[0].onclick = new Function("processClick(this)");
...
function processClick(div) {
    ...
    for (i = 0; i < catlength; i++) {
        ...
        else if (head == div && contdiv.style.display == 'inline-block') {
            //initSlide(cont, 1);
        }
        else if (contdiv.style.display == 'inline-block') {
            initSlide(cont, -1);
        }
        /*
